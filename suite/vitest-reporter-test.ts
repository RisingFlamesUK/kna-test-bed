// suite/vitest-reporter-test.ts
import type { Reporter, RunnerTestFile, RunnerTask } from 'vitest';
import { createCI } from './components/ci.ts';
import * as fs from 'fs';
import * as path from 'path';
import { buildScenarioLogPath, buildSuiteLogPath } from './components/logger.ts';
...
export default class TestReporter implements Reporter {
  ...
  private firstEventByFile = new Map<string, { when: number; source: 'console' | 'task'; snippet: string }>();
  ...
  private getSuiteDetailPath(): string | null {
    const stamp = process.env.KNA_LOG_STAMP;
    if (!stamp) return null;
    return path.resolve('logs', stamp, 'suite-detail.json');
  }
  ...
  private isSuiteAreaByKey(fileKey: string): boolean {
    ...
    return /test\/e2e\/suite\.test\.ts$/i.test(abs);
  }

  private isSchemaRunnerAreaByKey(fileKey: string): boolean {
    const area = this.areas.get(fileKey);
    const abs = ((area as any)?.meta?.testFileAbs ?? area?.filePathAbs ?? '').replace(/\\/g, '/');
    return /test\/e2e\/scenarios\/_runner\/prompt-map\.schema\.test\.ts$/i.test(abs);
  }
  ...
  private loadNonScenarioSteps(fileKey: string): Array<{ severity: 'ok'|'warn'|'fail'|'skip'; message: string }>{
    try {
  ...
  private ensureNonScenarioStepsPrinted(fileKey: string, ensureHeaderFor: string) {
    ...
    for (let i = already; i < steps.length; i++) {
      const s = steps[i];
      const icon = s.severity === 'fail' ? '❌' : (s.severity === 'warn' ? '⚠️' : (s.severity === 'skip' ? '↩️' : '✅'));
  ...
  private startNonScenarioPolling(fileKey: string, ensureHeaderFor: string) {
    ...
    const t = setInterval(() => {
      if (this.closedAreas.has(fileKey)) { this.stopNonScenarioPolling(fileKey); return; }
  ...
  private printDefaultNonScenarioLogIfNeeded(fileKey: string, ensureHeaderFor: string) {
    ...
    const candidate = stamp ? buildScenarioLogPath(stamp, scenarioName) : path.join('logs', 'latest', 'e2e', `${scenarioName}.log`);
  ...
  private ensureScenarioStepsPrinted(fileKey: string, scenarioName: string, ensureHeaderFor: string) {
    // Core steps de-dupe per scenario
    let stepsSet = this.scenarioStepsPrinted.get(fileKey);
    if (!stepsSet) { stepsSet = new Set<string>(); this.scenarioStepsPrinted.set(fileKey, stepsSet); }
    ...
    if (!stepsSet.has(scenarioName)) {
      ...
      const toIcon = (sev: string | undefined) => sev === 'fail' ? '❌' : (sev === 'warn' ? '⚠️' : '✅');
      const toText = (sev: string | undefined, base: string) => sev === 'fail' ? `${base}: Failed` : (sev === 'warn' ? `${base}: WARN` : `${base}: OK`);
      linesToEmit.push(`- ${toIcon(steps.scaffold?.severity)} ${toText(steps.scaffold?.severity, 'scaffold')}`);
      linesToEmit.push(`- ${toIcon(steps.env?.severity)} ${toText(steps.env?.severity, 'env manifest checks')}`);
      linesToEmit.push(`- ${toIcon(steps.files?.severity)} ${toText(steps.files?.severity, 'files manifest checks')}`);
    }
    ...
    try {
      ...
      if (cfgPath && fs.existsSync(cfgPath)) {
        ...
        if (found?.tests?.mergeEnv?.env) {
          ...
          if (!skipSet) { skipSet = new Set<string>(); this.scenarioSkipPrinted.set(fileKey, skipSet); }
  ...
  private maybeActivate(filePath: string) {
    ...
    const suiteKey = this.fileQueue.find((k) => !this.closedAreas.has(k) && this.isSuiteAreaByKey(k));
  ...
  private closeArea(filePath: string) {
    ...
    if (buf) {
      for (const l of buf.lines) {
        if (/^-\s+log:\s+/i.test(l)) sawExplicitInFlush = true;
        this.ci.boxLine(l);
      }
      buf.lines.length = 0;
    ...
    if (genQ && genQ.length) {
      for (const l of genQ) { this.ci.boxLine(l); }
      genQ.length = 0;
      sawExplicitInFlush = true;
    }
      ...
      if (isScenarioArea && stamp) {
        ...
        if (fs.existsSync(e2eDir)) {
          ...
          if (files.length) {
            ...
            // fallback single
            const absLog = path.resolve('logs', stamp, 'e2e', `${testBase}.log`).replace(/\\/g, '/').replace(/ /g, '%20');
            this.ci.boxLine(`- log: file:///${absLog}`);
          }
        } else {
        ...
        const candidate = isSuite
          ? (stamp ? buildScenarioLogPath(stamp, 'suite-sentinel') : path.join('logs', 'latest', 'e2e', 'suite-sentinel.log'))
          : (stamp ? buildScenarioLogPath(stamp, scenarioName) : path.join('logs', 'latest', 'e2e', `${scenarioName}.log`));
        const absLog = path.isAbsolute(candidate) ? candidate : path.resolve(candidate);
        const url = absLog.replace(/\\/g, '/').replace(/ /g, '%20');
    ...
    if (this.isScenarioAreaByKey(key)) {
      ...
      const detail = this.loadScenarioDetail() || {};
      let passed = 0, failed = 0, warn = 0, skipped = 0;
      for (const s of scenNames) {
        const d = detail?.[s] || {};
        const sev = (k:string)=> (d?.[k]?.severity as string|undefined) ?? undefined;
        const add = (kk: string)=> {
          const v = sev(kk);
          if (v === 'fail') failed += 1; else if (v === 'warn') warn += 1; else passed += 1;
        };
        add('scaffold'); add('env'); add('files');
      }
  ...
  private performClose(filePath: string) {
    ...
    if (this.activeFileKey === key || (!this.activeFileKey && head === key)) {
      ...
      while (true) {
        ...
        if (!nextArea) break;
        if (this.finished.has(next) || (nextArea.meta.finishedTests >= nextArea.meta.totalTests && nextArea.meta.totalTests > 0)) {
  ...
  onCollected(files: RunnerTestFile[]): void {
    ...
    if (!this.queueFinalized) {
      ...
      const isSchema = (abs:string)=> /test\/e2e\/scenarios\/_runner\/prompt-map\.schema\.test\.ts$/i.test(toPosix(abs));
      const isScenario = (abs:string)=> /test\/e2e\/scenarios\/.+\/.+\.test\.ts$/i.test(toPosix(abs));
  ...
  const alreadyPrinted = this.printedTests.has(String(id));
      ...
      if (state !== 'pass' && state !== 'fail' && state !== 'skip' && state !== 'run' && state !== 'running') continue;
  ...
  this.maybeActivate(filePath);
      ...
      if ((state === 'run' || state === 'running') && !alreadyPrinted) {
        ...
        if (isScenarioArea) {
          ...
          if (!set) { set = new Set<string>(); this.bulletsByFile.set(key, set); }
          if (!set.has(task.name || '')) {
            set.add(task.name || '');
            const bullet = `• Testing ${scenLabel}...`;
            if (this.activeFileKey === key) { this.ensureHeader(filePath); this.ci.boxLine(bullet); }
      ...
      const duration: number | undefined = (result?.duration ?? (task as any)?.result?.duration) as any;
        ...
        if (this.activeFileKey === key) {
          ...
          if (isScenarioArea2) {
            if (scenResolved) {
              ...
              if (!seen) { seen = new Set<string>(); this.printedLogLinesByFile.set(key, seen); }
              if (!seen.has(line)) {
                this.ci.boxLine(line);
                seen.add(line);
              ...
              if (sm) { sm.delete(name); sm.delete(scenResolved); }
            }
          } else {
          ...
          if (q && q.length) { for (const l of q) this.ci.boxLine(l); q.length = 0; }
        }
        else {
          ...
          if (scenResolved) {
            ...
            const tmpBuf: string[] = [];
            const originalBuf = this.buffers.get(key)!;
            // Temporarily print into tmpBuf by hijacking ci.boxLine? Simpler: push placeholder; we'll call ensureScenarioStepsPrinted which writes to buffer
          ...
          if (isScenarioArea2 && scenResolved) {
            ...
            const sm = this.scenarioLogsByFile.get(key);
            let ln = sm?.get(name) ?? (scenResolved ? sm?.get(scenResolved) : undefined);
            if (ln) { lines.push(ln); sm!.delete(name); if (scenResolved) sm!.delete(scenResolved); }
          }
          // Generic logs
          const q = this.deferredLogsByFile.get(key);
          if (q && q.length) { lines.push(...q); q.length = 0; }
          buf.lines.push(...lines);
        }
  // Count only scenario tests toward scenario footer totals
  if (area && this.isScenarioAreaByKey(key)) { area.counts.total += 1; area.counts.passed += 1; }
      } else if (state === 'fail') {
        if (this.activeFileKey === key) {
          this.ensureHeader(filePath);
          if (!this.isScenarioAreaByKey(key)) {
          ...
          if (isScenarioArea2 && scenResolved) {
            ...
            const sm = this.scenarioLogsByFile.get(key);
            let ln = sm?.get(name) ?? (scenResolved ? sm?.get(scenResolved) : undefined);
            if (ln) { this.ci.boxLine(ln); sm!.delete(name); if (scenResolved) sm!.delete(scenResolved); }
          }
          const q = this.deferredLogsByFile.get(key);
          if (q && q.length) { for (const l of q) this.ci.boxLine(l); q.length = 0; }
        }
          ...
          if (isScenarioArea2 && scenResolved) {
            ...
            let seen = this.printedLogLinesByFile.get(key);
            if (!seen) { seen = new Set<string>(); this.printedLogLinesByFile.set(key, seen); }
            if (!seen.has(line)) { lines.push(line); seen.add(line); const bufX = this.buffers.get(key); if (bufX) bufX.hasExplicitLog = true; }
            const sm = this.scenarioLogsByFile.get(key);
            if (sm) { sm.delete(name); sm.delete(scenResolved); }
          } else {
            const sm = this.scenarioLogsByFile.get(key);
  ...
  if (area && this.isScenarioAreaByKey(key)) { area.counts.total += 1; area.counts.failed += 1; }
      } else if (state === 'skip') {
        if (this.activeFileKey === key) {
          this.ensureHeader(filePath);
          if (!this.isScenarioAreaByKey(key)) {
          ...
          if (isScenarioArea && scenResolved) {
            ...
            let seen = this.printedLogLinesByFile.get(key);
            if (!seen) { seen = new Set<string>(); this.printedLogLinesByFile.set(key, seen); }
            if (!seen.has(line)) { this.ci.boxLine(line); seen.add(line); const bufX = this.buffers.get(key); if (bufX) bufX.hasExplicitLog = true; }
            const sm = this.scenarioLogsByFile.get(key);
            if (sm) { sm.delete(name); sm.delete(scenResolved); }
          } else {
            const sm = this.scenarioLogsByFile.get(key);
  ...
  if (area && this.isScenarioAreaByKey(key)) { area.counts.total += 1; area.counts.skipped += 1; }
      }
  ...
  onUserConsoleLog(log: { content: string; task?: RunnerTask }): void {
    ...
    let filePath = '';
    if (!log.task?.file) {
      if (log.content.includes('📝 Logs for this run')) { this.ci.write('\n' + log.content); }
  ...
  const isScenario = this.isScenarioAreaByKey(key);
        ...
        if (isScenario) {
          ...
          if (m) {
            ...
            if (!map) { map = new Map<string,string>(); this.scenarioLogsByFile.set(key, map); }
            ...
            if (lastPrinted && testTitle === lastPrinted) {
              if (this.activeFileKey === key) { this.ensureHeader(filePath); this.ci.boxLine(line); }
        ...
        let seen = this.printedLogLinesByFile.get(key);
        if (!seen) { seen = new Set<string>(); this.printedLogLinesByFile.set(key, seen); }
        if (seen.has(line)) { buf.hasExplicitLog = true; return; }
        if (testId && this.printedTests.has(testId)) {
          ...
          if (!q) { q = []; this.deferredLogsByFile.set(key, q); }
          q.push(line);
        }
        buf.hasExplicitLog = true;
        return;
        ...
        if (this.activeFileKey === key) { this.ensureHeader(filePath); this.ci.boxLine(`- ${trimmed}`); }
  ...
  onFinished(_files: RunnerTestFile[]): void {
    ...
    const isSchema = (p:string)=> /test\/e2e\/scenarios\/_runner\/prompt-map\.schema\.test\.ts$/i.test(toPosix(p));
    ...
    if (this.debugEnabled()) {
      ...
      for (const e of entries) {
        ...
        this.ci.boxLine(`${String(e.when).padStart(5, ' ')}ms — ${name} — ${e.source}: ${e.snippet}`);
      }
