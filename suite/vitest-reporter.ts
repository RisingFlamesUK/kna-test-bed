...
export default class HierarchicalReporter implements Reporter {
  ...
  // Track file-level durations (total time for all tests in each file)
  ...
  onTaskUpdate(packs: any[]): void {
    ...
    for (const [taskId, result] of packs) {
      ...
      if (state === 'pass' || state === 'fail' || state === 'skip') {
        // Test completed - track duration
        const duration: number | undefined = result?.duration ?? (task as any)?.result?.duration;
        if (duration != null) {
          const currentDuration = this.fileDurations.get(fileKey) || 0;
          this.fileDurations.set(fileKey, currentDuration + duration);
        }
  ...
  private printAreaFooter(hier: FileHierarchy): void {
    ...
    // Get file duration
    const fileDuration = this.fileDurations.get(fileKey) || 0;
    const durationText = fileDuration > 0 ? `, ${Math.round(fileDuration)}ms` : '';

    this.ci.boxEnd(
      `(Tests: ${testCount}, passed: ${passed}, failed: ${failed}, warning: ${warn}, skipped: ${skipped}${durationText})`,
    );
  }
